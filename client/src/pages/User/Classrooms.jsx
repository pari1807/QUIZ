...
const Classrooms = () => {
  ...
  // Get all matching videos when searching
  const searchedVideos = useMemo(() => {
    ...
    return matchingVideos.sort((a, b) => {
      ...
      >
        ...
        {search && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-slate-900/60 backdrop-blur-2xl border border-slate-700/50 rounded-2xl p-6"
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                  {searchedVideos.length} video{searchedVideos.length !== 1 ? 's' : ''} found
                </p>
              </div>
              <button
                onClick={() => setSearch('')}
            ...
            {searchedVideos.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-lg font-medium text-slate-400">No videos found</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {searchedVideos.map((v, idx) => (
                  <motion.div
                    key={v._id || v.id || idx}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: idx * 0.05 }}
                    className="group bg-slate-800/60 border border-slate-700/50 rounded-xl overflow-hidden hover:border-cyan-500/50 transition-all"
                  >
                    {/* Thumbnail */}
                    <div className="aspect-video bg-gradient-to-br from-slate-800 to-slate-900 relative flex items-center justify-center">
                      <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-cyan-500/30 transition-all">
                        <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" />
                        </svg>
                      </div>
                      <span className="absolute bottom-2 left-2 px-2 py-1 rounded text-xs bg-slate-900/80 text-slate-400">
                        📁 {v.playlistName}
                      </span>
                    </div>
